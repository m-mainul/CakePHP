<?php $this->assign('title', 'Ads User');?>
<?php  
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel"> <?php echo $this->Session->flash(); ?>
        <div class="x_content"> <br />
          ...
          <?php echo $this->Form->create('AdsUser',array('url' => 'edit','method'=>'post','name'=>'addForm','class'=>'form-horizontal form-label-left',"enctype"=>"multipart/form-data") ); 
          echo $this->Form->hidden('AdsUser.id');
      ?>
       ...
       "1"=>'Yes',
       "0"=>'No'
      ); if(empty($existing_users)) $existing_users = ''; echo $this->Form->input('AdsUser.is_exist_user', array('options'=>$options,'required'=>'true','label' => false, 'div' => false,'class' => 'form-control col-md-7 col-xs-12 selectpicker show-menu-arrow','title'=>'Select Existing User'));?> </div>
         </div>
...
function change_country(){
  ...
  jQuery.ajax({
                                    ...
                                    cache: false,
                                    dataType: 'text',
                                    url: '<?php echo $path;?>/Adsusers/getState/' + id,
...
function change_state(){
  ...
  jQuery.ajax({
                                    ...
                                    cache: false,
                                    dataType: 'text',
                                    url: '<?php echo $path;?>/Adsusers/getCity/' + id,
                                    success: function(response) 
                  {
...
function check_mail(){
  ...
  jQuery.ajax({
                                    ...
                                    cache: false,
                                    dataType: 'json',
                                    url: '<?php echo $path;?>/Adsusers/checkUser/' + id,
                                    success: function(response) 
                  {   
...
function check_mobile(){
  ...
  jQuery.ajax({
                                    ...
                                    cache: false,
                                    dataType: 'json',
                                    url: '<?php echo $path;?>/Adsusers/checkmobile/' + val,
                                    success: function(response) 
                  {   
                     if(response.status=="error")
                     {
...
jQuery(function($){
   $("#AdsUserAnniversaryDate").mask("99-99-9999",{placeholder:"dd-mm-yyyy"});
    $("#AdsUserDob").mask("99-99-9999",{placeholder:"dd-mm-yyyy"});
  });
</script>