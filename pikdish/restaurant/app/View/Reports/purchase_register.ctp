 ...
 <div class="table-responsive">
	<div class="row filter-date">
		...
		<div class="col-lg-2 export-to-excel">
			<button onclick="window.location.href='<?php echo Router::url(array('controller'=>'Reports', 'action'=>'exportPurchase'))?>'">Export to Excel</button>
		</div>
...
var mygrid = jQuery("#list").jqGrid({
	url:'<?php echo $path ?>reports/jsonPurchase?>',
	datatype: "json",
   	colNames:['Vch Date','Vch No','Vendor Name','Bill No', 'Bill Date','Tax Amt','Bill Amt',],
   	colModel:
	[
	    {name:'vch_date',index:'PurchaseH.vch_date', width:300,stype:'text'},
	    {name:'vch_no',index:'PurchaseH.vch_no', width:300,stype:'text'},
   		{name:"vendor_name",index:"Vendor.name", width:300,stype:'text'},
   		{name:"bill_no",index:"PurchaseH.bill_no", width:100,stype:'text'},
   		{name:"bill_date",index:"PurchaseH.bill_date", width:200,stype:'text'},
   		{name:"tax_amt",index:"PurchaseH.tax_amt", width:150,stype:'text'},
   		{name:"bill_amt",index:"PurchaseH.bill_amt", width:150,stype:'text'}
	],
...
$(document).ready(function(){
	...
	$('.filter-date').on('click','.date-filter',function(){
		...
		jQuery("#list").jqGrid('setGridParam',{url:"<?php echo $path ?>reports/jsonPurchase?start_date="+start_date+"&end_date="+end_date,page:1}).trigger("reloadGrid");
