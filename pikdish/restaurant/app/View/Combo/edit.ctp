  ...
  <div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel"> <?php echo $this->Session->flash(); ?>
        <div class="x_content">
          <?php echo $this->Form->create('ComboOffer',array('url'=>'edit','method'=>'post','name'=>'editForm','class'=>'form-horizontal form-label-left') ); 
		         echo $this->Form->hidden('ComboOffer.id',array("value"=>$this->request->data[0]['ComboOffer']['id'])) ;
                ...
                <table class="table table-bordered table-hover" id="tab_logic">
				   <thead>
                    <tr  style="background:#FF9">
					  ...
					  <th width="35%" class="text-center">Combo Offer Name*</th>
                  ...
                  <tbody>
                    <tr id='addr1' class="tdr">
                        ...
                        <td>
                          <input name="data[ComboOffer][offer_name]" required="required" class="form-control col-md-7 col-xs-12" autofocus="autofocus" placeholder="Offer Name" value="<?=$this->request->data[0]['ComboOffer']['offer_name']?>" >
                        </td>
                        <td>
             <input name="data[ComboOffer][start_date]" required="required" class="form-control col-md-7 col-xs-12"  placeholder="Start Date" id="ComboOfferStartDate" value="<?=date("d-m-Y",strtotime($this->request->data[0]['ComboOffer']['start_date']))?>" onchange="check_date()">
                        </td>
                        <td>
                            <input name="data[ComboOffer][end_date]" required="required" class="form-control col-md-7 col-xs-12" autofocus="autofocus" placeholder="End Date" id="ComboOfferEndDate" value="<?=date("d-m-Y",strtotime($this->request->data[0]['ComboOffer']['end_date']))?>" onchange="check_date()" >
                        </td>
                        <td>
                         <input name="data[ComboOffer][amt]" required="required" class="form-control col-md-7 col-xs-12"  placeholder="Amount"  value="<?=$this->request->data[0]['ComboOffer']['amt']?>" type="number" min="1">
                         </td>
                    ...
                    <tr>
                      <td width="100%" class="" colspan="4">
                       ...
                       <!--<td >
                           ...
                           <input type="checkbox" name="data[ComboOffer][gst_applicability]" value="1" style="width:15%" onchange="if(this.checked){$('#_gst_0').attr('disabled',false).focus().val('<?=$this->request->data[0]['ComboOffer']['gst_precentage']?>');}else{$('#_gst_0').attr('disabled',true).val('');}" tabindex="1" <? if($this->request->data[0]['ComboOffer']['gst_applicability'] == 1){echo "checked";} 	?>   /><br /><br /><input id="_gst_0" name="data[ComboOffer][gst_precentage]" required="required" class="form-control col-md-7 col-xs-12"  placeholder="GST Precentage"    step="0.01" type="number" min=0 <? if($this->request->data[0]['ComboOffer']['gst_applicability'] == 1){echo "value='".$this->request->data[0]['ComboOffer']['gst_precentage']."'";}else {echo 'disabled="disabled"' ;} 	?> >
                       </td> -->
                    </tr>
...
jQuery(function($){
  ...
  function check_date()
  {
	 ...
	 else
	 {
		...
		var str = "<tr>";i=0;
        <? 
	     for($i = 0;$i < count($this->request->data[0]['ComboItems']) ; $i++) 
	     {  
	    ?>
		 el = $('#ItemsRatePortionId option[value="<?=$this->request->data[0]['ComboItems'][$i]['items_rate_id']?>"]').attr('selected', true); 
		 lname = el.text(); 
		 sub_text =el.attr('data-subtext'); 
	     if(i!=0 && i%3==0) {str+= "</tr><tr>" ;}
		 str+='<td><label>'+lname+'</label> <span>'+sub_text+'</span><input type="number" name="data['+(i+1)+'][ComboItems][qty]" value="<?=$this->request->data[0]['ComboItems'][$i]['qty']?>"  required placeholder="quantity" class="form-control col-md-7 col-xs-12 tmp" min="1" /><input type="hidden" name="data['+(i+1)+'][ComboItems][items_rate_id]" value="<?=$this->request->data[0]['ComboItems'][$i]['items_rate_id']?>" /></td>';
