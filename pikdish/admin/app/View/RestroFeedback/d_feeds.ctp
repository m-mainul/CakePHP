...
<div class="title_left" style="margin-left:5px; width:40%">
   <button type="button" style="border-radius:5px;" class="btn btn-success btn-lg " onclick="get_selected(1)" >Verify Selected</button>      
</div>
...
<div class="title_right table-responsive" style="float:right; text-align:right; margin-right:5px">
      <nav aria-label="Page navigation ">
          <ul class="pagination pagination-lg" >
            ...
            <li ><a href="<?=$path?>restro_feedback/d_feedslist/<?=$restor_id?>"><span class="glyphicon glyphicon glyphicon-th-large" aria-hidden="true"></span></a></li>
          </ul>
        </nav>
</div>
</div>
...
var mygrid = jQuery("#list").jqGrid({
	url:'<?php echo $path; ?>Restro_Feedback/feedjson/<?=$restor_id?>/2',
	datatype: "json",
	...
	rownumWidth: 40,
	shrinkToFit: true,
	caption:"Deleted Customer Feedbacks List",
...
function Delete_feedback(id,type = 1)
{
			...
			buttons: {
				"Verify Feedback": function() {
					...
					jQuery.ajax({
                                    ...
                                    cache: false,
                                    dataType: 'json',
                                    url: '<?php echo $path;?>/restro_feedback/varify/' + id+'/'+type+'/1' ,
                                    success: function(response) {
										if(d==1 || ding == 1)
										{
											 ding=0;
