...
class ReportsTable8Controller extends AppController {
    ...
    public function show()
    {
        ...
        if ($this -> request -> is('post')) {
            ...
            for($i=10; $i <= 33; $i++)
            {
                ...
                $res = $this->_make_query($i, " R.Q22_IS_FIRE_SECURED = 1 ");
                $data[$j]['FIRE_FIGHTING'] = $res[0]['TOTAL_EST'];

                $res = $this->_make_query($i, " R.Q23_IS_GARBAGE_PROPER = 1 ");
                $data[$j]['WASTE_MANAGEMENT'] = $res[0]['TOTAL_EST'];

                $res = $this->_make_query($i, " R.Q24_IS_TOILET_AVAILABLE = 1 ");
                $data[$j]['TOILET_FACILITY'] = $res[0]['TOTAL_EST'];

                $res = $this->_make_query($i, " R.Q24_IS_TOILET_AVAILABLE = 1 AND Q24_IS_LADIES_TOILET_AVAILABLE = 1 ");
                $data[$j]['FEMALE_TOILET'] = $res[0]['TOTAL_EST'];
                $j++;
            }
        } 
