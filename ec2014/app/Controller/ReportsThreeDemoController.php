...
class ReportsThreeDemoController extends AppController {
	...
	public function tpe_tbl_three_one_demo() {
		...
		if ($this -> request -> is('post')) {
			...
			$where_urban1 = $where . " AND ques_rmo_code IN(2,3) ";  // Municipal 
			$where_urban2 = $where . " AND ques_rmo_code IN(9) ";    // CityCorp.
			...
			$where_urban_parmanent1 = $where_urban1 . " AND Q4_UNIT_TYPE = 2 AND Q4_UNIT_ORG_TYPE = 1 ";
			$where_urban_temporary1 = $where_urban1 . " AND Q4_UNIT_TYPE = 2 AND Q4_UNIT_ORG_TYPE = 2 ";
			$where_urban_household1 = $where_urban1 . " AND Q4_UNIT_TYPE = 1 ";

			$where_urban_parmanent2 = $where_urban2 . " AND Q4_UNIT_TYPE = 2 AND Q4_UNIT_ORG_TYPE = 1 ";
			$where_urban_temporary2 = $where_urban2 . " AND Q4_UNIT_TYPE = 2 AND Q4_UNIT_ORG_TYPE = 2 ";
			$where_urban_household2 = $where_urban2 . " AND Q4_UNIT_TYPE = 1 ";
           ...
           */
			...
			//#############################   PART TWO:URBAN ####################################
			//ROW ONE URBAN TOTAL
			$total_urban_est1 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban1);
			$total_urban_est1 = $total_urban_est1[0][0]['total_est'];

			$total_urban_est_person1 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban1);
			$total_urban_est_person1 = $total_urban_est_person1[0][0]['total_person_engaged'];

			$total_urban_est_male1 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban1);
			$total_urban_est_male1 = $total_urban_est_male1[0][0]['total_male_engaged'];

			$total_urban_est_female1 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban1);
			$total_urban_est_female1 = $total_urban_est_female1[0][0]['total_female_engaged'];
			#-------------------Citycorporation
			$total_urban_est2 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban2);
			$total_urban_est2 = $total_urban_est2[0][0]['total_est'];

			$total_urban_est_person2 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban2);
			$total_urban_est_person2 = $total_urban_est_person2[0][0]['total_person_engaged'];

			$total_urban_est_male2 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban2);
			$total_urban_est_male2 = $total_urban_est_male2[0][0]['total_male_engaged'];

			$total_urban_est_female2 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban2);
			$total_urban_est_female2 = $total_urban_est_female2[0][0]['total_female_engaged'];

			//ROW TWO URBAN PARMANENT
			$total_urban_parmanent_est1 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent1);
			$total_urban_parmanent_est1 = $total_urban_parmanent_est1[0][0]['total_est'];

			$total_urban_parmanent_est_person1 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent1);
			$total_urban_parmanent_est_person1 = $total_urban_parmanent_est_person1[0][0]['total_person_engaged'];

			$total_urban_parmanent_est_male1 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent1);
			$total_urban_parmanent_est_male1 = $total_urban_parmanent_est_male1[0][0]['total_male_engaged'];

			$total_urban_parmanent_est_female1 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent1);
			$total_urban_parmanent_est_female1 = $total_urban_parmanent_est_female1[0][0]['total_female_engaged'];
			#-------------Citycorporation
			$total_urban_parmanent_est2 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent2);
			$total_urban_parmanent_est2 = $total_urban_parmanent_est2[0][0]['total_est'];

			$total_urban_parmanent_est_person2 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent2);
			$total_urban_parmanent_est_person2 = $total_urban_parmanent_est_person2[0][0]['total_person_engaged'];

			$total_urban_parmanent_est_male2 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent2);
			$total_urban_parmanent_est_male2 = $total_urban_parmanent_est_male2[0][0]['total_male_engaged'];

			$total_urban_parmanent_est_female2 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_parmanent2);
			$total_urban_parmanent_est_female2 = $total_urban_parmanent_est_female2[0][0]['total_female_engaged'];

			//ROW THREE URBAN TEMPORARY
			$total_urban_temporary_est1 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary1);
			$total_urban_temporary_est1 = $total_urban_temporary_est1[0][0]['total_est'];

			$total_urban_temporary_est_person1 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary1);
			$total_urban_temporary_est_person1 = $total_urban_temporary_est_person1[0][0]['total_person_engaged'];

			$total_urban_temporary_est_male1 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary1);
			$total_urban_temporary_est_male1 = $total_urban_temporary_est_male1[0][0]['total_male_engaged'];

			$total_urban_temporary_est_female1 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary1);
			$total_urban_temporary_est_female1 = $total_urban_temporary_est_female1[0][0]['total_female_engaged'];
			#------citycorp  
			$total_urban_temporary_est2 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary2);
			$total_urban_temporary_est2 = $total_urban_temporary_est2[0][0]['total_est'];

			$total_urban_temporary_est_person2 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary2);
			$total_urban_temporary_est_person2 = $total_urban_temporary_est_person2[0][0]['total_person_engaged'];

			$total_urban_temporary_est_male2 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary2);
			$total_urban_temporary_est_male2 = $total_urban_temporary_est_male2[0][0]['total_male_engaged'];

			$total_urban_temporary_est_female2 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_temporary2);
			$total_urban_temporary_est_female2 = $total_urban_temporary_est_female2[0][0]['total_female_engaged'];

			//ROW FOUR URBAN HOUSEHOLD
			$total_urban_household_est1 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban_household1);
			$total_urban_household_est1 = $total_urban_household_est1[0][0]['total_est'];

			$total_urban_household_est_person1 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_household1);
			$total_urban_household_est_person1 = $total_urban_household_est_person1[0][0]['total_person_engaged'];

			$total_urban_household_est_male1 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_household1);
			$total_urban_household_est_male1 = $total_urban_household_est_male1[0][0]['total_male_engaged'];

			$total_urban_household_est_female1 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_household1);
			$total_urban_household_est_female1 = $total_urban_household_est_female1[0][0]['total_female_engaged'];
			#----citycorp 
			$total_urban_household_est2 = $this -> Report -> query("SELECT COUNT(*) AS total_est FROM BBSEC2013_REPORTS WHERE " . $where_urban_household2);
			$total_urban_household_est2 = $total_urban_household_est2[0][0]['total_est'];

			$total_urban_household_est_person2 = $this -> Report -> query("SELECT SUM(total_person_engaged) AS total_person_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_household2);
			$total_urban_household_est_person2 = $total_urban_household_est_person2[0][0]['total_person_engaged'];

			$total_urban_household_est_male2 = $this -> Report -> query("SELECT SUM(total_male_engaged) AS total_male_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_household2);
			$total_urban_household_est_male2 = $total_urban_household_est_male2[0][0]['total_male_engaged'];

			$total_urban_household_est_female2 = $this -> Report -> query("SELECT SUM(total_female_engaged) AS total_female_engaged FROM BBSEC2013_REPORTS WHERE " . $where_urban_household2);
			$total_urban_household_est_female2 = $total_urban_household_est_female2[0][0]['total_female_engaged'];
       ...
       # Sending Query Result To View
		...
		# -------URBAN----------
		'total_urban_est1', 'total_urban_est_person1', 'total_urban_est_male1', 'total_urban_est_female1',
		'total_urban_est2', 'total_urban_est_person2', 'total_urban_est_male2', 'total_urban_est_female2',
		# -----------------
		'total_urban_parmanent_est1', 'total_urban_parmanent_est_person1', 'total_urban_parmanent_est_male1', 'total_urban_parmanent_est_female1',
		'total_urban_parmanent_est2', 'total_urban_parmanent_est_person2', 'total_urban_parmanent_est_male2', 'total_urban_parmanent_est_female2',
		# -----------------
		'total_urban_temporary_est1', 'total_urban_temporary_est_person1', 'total_urban_temporary_est_male1', 'total_urban_temporary_est_female1',
		'total_urban_temporary_est2', 'total_urban_temporary_est_person2', 'total_urban_temporary_est_male2', 'total_urban_temporary_est_female2',
		# -----------------
		'total_urban_household_est1', 'total_urban_household_est_person1', 'total_urban_household_est_male1', 'total_urban_household_est_female1',
		'total_urban_household_est2', 'total_urban_household_est_person2', 'total_urban_household_est_male2', 'total_urban_household_est_female2',
		# -------RURAL----------
