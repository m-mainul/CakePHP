...
class UsersController extends AuthakeAppController {
	...
	var $paginate = array('limit' => 100, 'order' => array('User.login' => 'asc'));//var $scaffold;
	...
	function index($tableonly = false) {
		 ...
		 */
		 ...
		 if ($this -> request -> is('post') && $this -> request ->data['User']['login'] != "") 	{
			$conditions = array('User.login LIKE' => '%'.$this -> request ->data['User']['login'].'%');
		}
		else {
			$conditions = array();
		}
		...
		$users = $this->paginate(null, $conditions);
		
		
		
		$this -> set(compact('users', 'tableonly'));
