...
class UserController extends AuthakeAppController {
	...
	//var $scaffold;
	function denied() {
		$this -> Session -> setFlash(__('You do not have permission to access this page...', true));
			...
			if ($this -> User -> save($user['User'])) {
				switch ($state) {
					...
					case 2 :
						...
						break;
					default :
						$this -> Session -> setFlash(__('The User profile has been saved'), 'success', array('plugin' => 'Authake'));
				}
			}
	...
	function reset_password() {
		...
		if ($this -> request -> is('post')) {		
			...
			if ($this -> request -> data['User']['loginoremail'] == "") {
				$this -> Session -> setFlash(__('User Id cannot be null.'));
				$this -> redirect(array('action' => 'reset_password'));
			}

			if ($this -> request -> data['User']['loginpassword'] == "" || strlen($this -> request -> data['User']['loginpassword']) <= 6) {
				$this -> Session -> setFlash(__('Password length must be six digit long. Please, try again.'));

				$this -> redirect(array('action' => 'reset_password'));
			}

			$reset_status = $this -> User -> updateAll(array('User.password' => "'" . md5($this -> request -> data['User']['loginpassword']) . "'"), array('User.login' => $this -> request -> data['User']['loginoremail']));
		...
		//$users = $this -> User -> find('all', array('fields' => array('login')));
